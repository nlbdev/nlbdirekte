...
function SmilPlayer() {
		...
		if (this.doneLoading) {
			if (!paused) {
				if (audioObject === null || audioObject.readyState === 0 || audioObject.readyState === 2) {
					// no audio, or audio failed to load; use timers to update currentTime when no audio is playing
					currentTime += (thisThreadRun-lastThreadRun)/1000.;
				}
				else if (audioObject.readyState === 3) {
					// audio is ready; use audio object to calculate currentTime
	...
	function update() {
		...
		if (skipTo >= 0) {
			if (!paused && consoleLog && debugIsSameSrc && !(typeof audioObject === 'object' && typeof audioObject.url === 'string'))
				console.log('isSameSrc #1: audioObject:'+typeof audioObject+(typeof audioObject==='object'?(' audioObject.url:'+typeof audioObject.url):''));
		...
		if (audioObject === null) {
			...
			if (!!soundManager && soundManager.ok()) {
				...
				audioObject = soundManager.createSound({
					id: 'id'+getAttr(smilNode,'s',''),
					url: this.server.getUrl(getAttr(smilNode,'s','')),
					volume: Math.floor(volume*100),
					position: Math.max(Math.floor((getAttr(smilNode,'B',-1) + (currentTime - getAttr(smilNode,'b',-1)))*1000.),1),
					autoLoad: true,
			...
			if (smilNode === null) {
				...
				if (consoleLog && !paused && debugIsSameSrc)
					console.log('isSameSrc #2: audioObject:'+typeof audioObject+(typeof audioObject==='object'?(' audioObject.url:'+(typeof audioObject.url==='string'?audioObject.url:typeof audioObject.url)):''));
				if (isSameSrc(audioObject.url,this.server.getUrl(getAttr(smilNode,'s','')))) {
					...
					if (Math.abs(audioObjectBegin+audioObject.position/1000.) - currentTime > inaccurateTimeMeasurement) {
						...
						if (consoleLog && debugCurrentTime) console.log('#2B audioObject.position/1000. = '+currentTime+' - '+audioObjectBegin+' = '+audioObject.position/1000.);
						//audioObject.resume(); // update position
						//if (paused) {
					...
					if (audioObject.position/1000. < getAttr(smilNode,'B',-1)-inaccurateTimeMeasurement ||
						audioObject.position/1000. > getAttr(smilNode,'E',-1)+inaccurateTimeMeasurement) {
							...
							if (consoleLog && debugCurrentTime) console.log('if ('+getAttr(smilNode,'B',-1)+' > '+audioObject.position/1000.+' || '+audioObject.position/1000.+' > '+(getAttr(smilNode,'B',-1)+inaccurateTimeMeasurement)+')');
							audioObject.setPosition(Math.floor(getAttr(smilNode,'B',-1)*1000.));
						...
						//if (consoleLog) console.log('paused:'+paused+' , audioObject.paused:'+audioObject.paused);
						//if (consoleLog) console.log(audioObject);
	...
	this.setVolume = function(vol) {
		...
		volume = vol;
		if (audioObject !== null) {
			audioObject.setVolume(Math.floor(volume*100.));
		}
	}

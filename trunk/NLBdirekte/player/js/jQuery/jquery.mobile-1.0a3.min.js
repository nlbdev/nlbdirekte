...
boxShadow:!!c("boxShadow")&&!i,scrollTop:("pageXOffset"in window||"scrollTop"in document.documentElement||"scrollTop"in f[0])&&!e,dynamicBaseTag:function(){var h=location.protocol+"//"+location.host+location.pathname+"ui-dir/",k=a("head base"),j=null,o="";if(k.length)o=k.attr("href");else k=j=a("<base>",{href:h}).appendTo("head");var q=a("<a href='testurl'></a>").prependTo(f)[0].href;k[0].href=o?o:location.pathname;j&&j.remove();return q.indexOf(h)===0}()});f.remove();a.support.boxShadow||a("html").addClass("ui-mobile-nosupport-boxshadow")})(jQuery);
(function(a,d){a.each("touchstart touchmove touchend orientationchange tap taphold swipe swipeleft swiperight scrollstart scrollstop".split(" "),function(e,i){a.fn[i]=function(h){return h?this.bind(i,h):this.trigger(i)};a.attrFn[i]=true});var c=a.support.touch,f=c?"touchstart":"mousedown",b=c?"touchend":"mouseup",g=c?"touchmove":"mousemove";a.event.special.scrollstart={enabled:true,setup:function(){function e(j,o){h=o;var q=j.type;j.type=h?"scrollstart":"scrollstop";a.event.handle.call(i,j);j.type=
q}var i=this,h,k;a(i).bind("touchmove scroll",function(j){if(a.event.special.scrollstart.enabled){h||e(j,true);clearTimeout(k);k=setTimeout(function(){e(j,false)},50)}})}};a.event.special.tap={setup:function(){var e=this,i=a(e);i.bind("mousedown touchstart",function(h){function k(n){if(n.type=="scroll")j=true;else{n=n.type=="touchmove"?n.originalEvent.touches[0]:n;if(Math.abs(w[0]-n.pageX)>10||Math.abs(w[1]-n.pageY)>10)j=true}}if(h.which&&h.which!==1||i.data("prevEvent")&&i.data("prevEvent")!==h.type)return false;
i.data("prevEvent",h.type);setTimeout(function(){i.removeData("prevEvent")},800);var j=false,o=true,q=h.target,s=h.originalEvent,w=h.type=="touchstart"?[s.touches[0].pageX,s.touches[0].pageY]:[h.pageX,h.pageY],m,r;r=setTimeout(function(){if(o&&!j){m=h.type;h.type="taphold";a.event.handle.call(e,h);h.type=m}},750);a(window).one("scroll",k);i.bind("mousemove touchmove",k).one("mouseup touchend",function(n){i.unbind("mousemove touchmove",k);a(window).unbind("scroll",k);clearTimeout(r);o=false;if(!j&&
q==n.target){m=n.type;n.type="tap";a.event.handle.call(e,n);n.type=m}})})}};a.event.special.swipe={setup:function(){var e=a(this);e.bind(f,function(i){function h(q){if(j){var s=q.originalEvent.touches?q.originalEvent.touches[0]:q;o={time:(new Date).getTime(),coords:[s.pageX,s.pageY]};Math.abs(j.coords[0]-o.coords[0])>10&&q.preventDefault()}}var k=i.originalEvent.touches?i.originalEvent.touches[0]:i,j={time:(new Date).getTime(),coords:[k.pageX,k.pageY],origin:a(i.target)},o;e.bind(g,h).one(b,function(){e.unbind(g,
...
add:function(o){var q=o.handler;o.handler=function(s){s.orientation=k();return q.apply(this,arguments)}}};k=function(){var o=document.documentElement;return o&&o.clientWidth/o.clientHeight<1.1?"portrait":"landscape"}})(jQuery);a.each({scrollstop:"scrollstart",taphold:"tap",swipeleft:"swipe",swiperight:"swipe"},function(e,i){a.event.special[e]={setup:function(){a(this).bind(i,a.noop)}}})})(jQuery);
(function(a,d,c){function f(j){j=j||location.href;return"#"+j.replace(/^[^#]*#?(.*)$/,"$1")}var b="hashchange",g=document,e,i=a.event.special,h=g.documentMode,k="on"+b in d&&(h===c||h>7);a.fn[b]=function(j){return j?this.bind(b,j):this.trigger(b)};a.fn[b].delay=50;i[b]=a.extend(i[b],{setup:function(){if(k)return false;a(e.start)},teardown:function(){if(k)return false;a(e.stop)}});e=function(){function j(){var n=f(),t=r(s);if(n!==s){m(s=n,t);a(d).trigger(b)}else if(t!==s)location.href=location.href.replace(/#.*/,
"")+t;q=setTimeout(j,a.fn[b].delay)}var o={},q,s=f(),w=function(n){return n},m=w,r=w;o.start=function(){q||j()};o.stop=function(){q&&clearTimeout(q);q=c};a.browser.msie&&!k&&function(){var n,t;o.start=function(){if(!n){t=(t=a.fn[b].src)&&t+f();n=a('<iframe tabindex="-1" title="empty"/>').hide().one("load",function(){t||m(f());j()}).attr("src",t||"javascript:0").insertAfter("body")[0].contentWindow;g.onpropertychange=function(){try{if(event.propertyName==="title")n.document.title=g.title}catch(v){}}}};
o.stop=w;r=function(){return f(n.location.href)};m=function(v,l){var p=n.document,u=a.fn[b].domain;if(v!==l){p.title=g.title;p.open();u&&p.write('<script>document.domain="'+u+'"<\/script>');p.close();n.location.hash=v}}}();return o}()})(jQuery,this);
...
CONTROL:17,DELETE:46,DOWN:40,END:35,ENTER:13,ESCAPE:27,HOME:36,INSERT:45,LEFT:37,MENU:93,NUMPAD_ADD:107,NUMPAD_DECIMAL:110,NUMPAD_DIVIDE:111,NUMPAD_ENTER:108,NUMPAD_MULTIPLY:106,NUMPAD_SUBTRACT:109,PAGE_DOWN:34,PAGE_UP:33,PERIOD:190,RIGHT:39,SHIFT:16,SPACE:32,TAB:9,UP:38,WINDOWS:91}});a(d);var f=a("html");a("head");var b=a.mobile.loadingMessage?a("<div class='ui-loader ui-body-a ui-corner-all'><span class='ui-icon ui-icon-loading spin'></span><h1>"+a.mobile.loadingMessage+"</h1></div>"):c;a.extend(a.mobile,
{pageLoading:function(g){if(g)f.removeClass("ui-loading");else{if(a.mobile.loadingMessage){g=a("."+a.mobile.activeBtnClass).first();b.appendTo(a.mobile.pageContainer).css({top:a.support.scrollTop&&a(d).scrollTop()+a(d).height()/2||g.length&&g.offset().top||100})}f.addClass("ui-loading")}},silentScroll:function(g){g=g||0;a.event.special.scrollstart.enabled=false;setTimeout(function(){d.scrollTo(0,g);a(document).trigger("silentscroll",{x:0,y:g})},20);setTimeout(function(){a.event.special.scrollstart.enabled=
true},150)}})})(jQuery,this);
(function(a,d){function c(l){if(i&&(!i.closest(".ui-page-active").length||l))i.removeClass(a.mobile.activeBtnClass);i=null}var f=a(window),b=a("html"),g=a("head"),e={get:function(l){if(l===d)l=location.hash;return e.stripHash(l).replace(/[^\/]*\.[^\/*]+$/,"")},getFilePath:function(l){var p="&"+a.mobile.subPageUrlKey;return l&&l.split(p)[0].split(s)[0]},set:function(l){location.hash=l},origin:"",setOrigin:function(){e.origin=e.get(location.protocol+"//"+location.host+location.pathname)},makeAbsolute:function(l){return e.get()+
...
1]},addNew:function(l,p){h.getNext()&&h.clearForward();h.stack.push({url:l,transition:p});h.activeIndex=h.stack.length-1},clearForward:function(){h.stack=h.stack.slice(0,h.activeIndex+1)},ignoreNextHashChange:true},k="[tabindex],a,button:visible,select:visible,input",j=null,o=[],q=false,s="&ui-state=dialog",w=g.children("base"),m=location.protocol+"//"+location.host,r=e.get(m+location.pathname),n=r;if(w.length){var t=w.attr("href");if(t)n=t.search(/^[^:\/]+:\/\/[^\/]+\/?/)===-1?t.charAt(0)==="/"?
m+t:r+t:t;n+=n.charAt(n.length-1)==="/"?" ":"/"}var v=a.support.dynamicBaseTag?{element:w.length?w:a("<base>",{href:n}).prependTo(g),set:function(l){v.element.attr("href",n+e.get(l))},reset:function(){v.element.attr("href",n)}}:d;e.setOrigin();a.fn.animationComplete=function(l){if(a.support.cssTransitions)return a(this).one("webkitAnimationEnd",l);else{setTimeout(l,0);return a(this)}};a.mobile.updateHash=e.set;a.mobile.path=e;a.mobile.base=v;a.mobile.urlstack=h.stack;a.mobile.urlHistory=h;a.mobile.changePage=
function(l,p,u,A,z){function I(){q=false;o.length>0&&a.mobile.changePage.apply(a.mobile,o.pop())}function T(){function C(){if(A!==false&&x){h.ignoreNextHashChange=false;e.set(x)}!M&&!N&&h.addNew(x,p);c();a.mobile.silentScroll(l.data("lastScroll"));var D=l,F=D.find(".ui-title:eq(0)");F.length?F.focus():D.find(k).eq(0).focus();y&&y.data("page")._trigger("hide",null,{nextPage:l});l.data("page")._trigger("show",null,{prevPage:y||a("")});a.mobile.activePage=l;O!==null&&O.remove();b.removeClass("ui-mobile-rendering");
I()}function J(D){a.mobile.pageContainer.addClass(D);K.push(D)}a.mobile.silentScroll();var G=f.scrollTop(),P=["flip"],K=[];if(x.indexOf("&"+a.mobile.subPageUrlKey)>-1)l=a("[data-url='"+x+"']");if(y){y.data("lastScroll",G);y.data("page")._trigger("beforehide",{nextPage:l})}l.data("page")._trigger("beforeshow",{prevPage:y||a("")});if(p&&p!=="none"){a.mobile.pageLoading(true);a.inArray(p,P)>=0&&J("ui-mobile-viewport-perspective");J("ui-mobile-viewport-transitioning");if(y)y.addClass(p+" out "+(u?"reverse":
""));l.addClass(a.mobile.activePageClass+" "+p+" in "+(u?"reverse":""));l.animationComplete(function(){l.add(y).removeClass("out in reverse "+p);y&&y.removeClass(a.mobile.activePageClass);C();a.mobile.pageContainer.removeClass(K.join(" "));K=[]})}else{a.mobile.pageLoading(true);y&&y.removeClass(a.mobile.activePageClass);l.addClass(a.mobile.activePageClass);C()}}function U(){if(j||l.data("role")==="dialog"){x=h.getActive().url+s;if(j){l.attr("data-role",j);j=null}}l.page()}var H=a.type(l)==="array",
L=a.type(l)==="object",y=H?l[0]:a.mobile.activePage;l=H?l[1]:l;var x=a.type(l)==="string"?e.stripHash(l):"",B=x,E,Q="get",V=false,O=null,R=h.getActive(),M=false,N=false;if(!(R&&h.stack.length>1&&R.url===x&&!H&&!L))if(q)o.unshift(arguments);else{q=true;if(z){var S;a.each(h.stack,function(C){if(this.url===x){M=C<h.activeIndex;N=!M;S=C}});h.activeIndex=S?S:h.activeIndex;if(M){u=true;p=p||R.transition}else if(N)p=p||h.getActive().transition}if(L&&l.url){x=l.url;E=l.data;Q=l.type;V=true;if(E&&Q==="get"){if(a.type(E)===
"object")E=a.param(E);x+="?"+E;E=d}}v&&v.reset();a(window.document.activeElement).add("input:focus, textarea:focus, select:focus").blur();if(x){l=a("[data-url='"+x+"']");B=e.getFilePath(x)}else{H=l.attr("data-url");L=e.getFilePath(H);if(H!==L)B=L}if(p===d)p=j&&j==="dialog"?"pop":a.mobile.defaultTransition;if(l.length&&!V){B&&v&&v.set(B);U();T()}else{if(l.length)O=l;a.mobile.pageLoading();a.ajax({url:B,type:Q,data:E,dataType:"html",success:function(C){var J=a("<div></div>"),G,P=/\bdata-url=["']?([^"'>]*)["']?/;
if(/.*(<[^>]*\bdata-role=["']?page["']?[^>]*>).*/.test(C)&&RegExp.$1&&P.test(RegExp.$1)&&RegExp.$1)G=RegExp.$1;if(G){v&&v.set(G);x=B=e.getFilePath(G)}else v&&v.set(B);J.get(0).innerHTML=C;l=J.find('[data-role="page"], [data-role="dialog"]').first();if(!a.support.dynamicBaseTag){var K=e.get(B);l.find("[src],link[href]").each(function(){var D=a(this).is("[href]")?"href":"src",F=a(this).attr(D);F.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(F)||a(this).attr(D,
K+F)})}l.attr("data-url",B).appendTo(a.mobile.pageContainer);U();setTimeout(function(){T()},0)},error:function(){a.mobile.pageLoading(true);c(true);v&&v.set(e.get());a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:a(window).scrollTop()+100}).appendTo(a.mobile.pageContainer).delay(800).fadeOut(400,function(){a(this).remove()});I()}})}}};a("form[data-ajax!='false']").live("submit",function(l){if(a.mobile.ajaxEnabled&&
a.mobile.ajaxFormsEnabled){var p=a(this).attr("method"),u=e.clean(a(this).attr("action"));if(!e.isExternal(u)){if(e.isRelative(u))u=e.makeAbsolute(u);a.mobile.changePage({url:u,type:p,data:a(this).serialize()},d,d,true);l.preventDefault()}}});a("a").live("click",function(l){var p=a(this),u=p.attr("href")||"#";u=e.clean(u);var A=p.is("[rel='external']"),z=e.isEmbeddedPage(u);A=e.isExternal(u)||A&&!z;z=p.is("[target]");var I=p.is("[data-ajax='false']");if(p.is("[data-rel='back']")){window.history.back();
return false}if(u!=="#"){i=p.closest(".ui-btn").addClass(a.mobile.activeBtnClass);if(A||I||z||!a.mobile.ajaxEnabled||!a.mobile.ajaxLinksEnabled){c(true);if(z)window.open(u);else if(I)return;else location.href=u}else{A=p.data("transition");z=(z=p.data("direction"))&&z==="reverse"||p.data("back");j=p.attr("data-rel");if(e.isRelative(u))u=e.makeAbsolute(u);u=e.stripHash(u);a.mobile.changePage(u,A,z)}}l.preventDefault()});f.bind("hashchange",function(){var l=e.stripHash(location.hash),p=a.mobile.urlHistory.stack.length===
0?false:d;if(!a.mobile.hashListeningEnabled||!h.ignoreNextHashChange||h.stack.length>1&&l.indexOf(s)>-1&&!a.mobile.activePage.is(".ui-dialog")){if(!h.ignoreNextHashChange)h.ignoreNextHashChange=true}else l?a.mobile.changePage(l,p,d,false,true):a.mobile.changePage(a.mobile.firstPage,p,true,false,true)})})(jQuery);
...
0;if(m){var n=m.offsetParent,t=document.body;for(r=m.offsetTop;m&&m!=t;){r+=m.scrollTop||0;if(m==n){r+=n.offsetTop;n=m.offsetParent}m=m.parentNode}}return r}function b(m){var r=a(window).scrollTop(),n=f(m[0]),t=m.css("top")=="auto"?0:parseFloat(m.css("top")),v=window.innerHeight,l=m.outerHeight(),p=m.parents(".ui-page:not(.ui-page-fullscreen)").length;if(m.is(".ui-header-fixed")){t=r-n+t;if(t<n)t=0;return m.css("top",p?t:r)}else{t=r+v-l-(n-t);return m.css("top",p?t:r+v-l)}}if(a.support.scrollTop){var g=
"inline",e=false,i,h,k=a.support.touch,j=k?"touchstart":"mousedown",o=k?"touchend":"mouseup",q=null,s=false,w=true;a(function(){a(document).bind(j,function(m){if(w)a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length||(q=g)}).bind(o,function(m){if(w)if(!a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length)if(!s){a.fixedToolbars.toggle(q);q=null}}).bind("scrollstart",function(m){if(!a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length){s=
true;if(q==null)q=g;if(e=(m=q=="overlay")||!!i){a.fixedToolbars.clearShowTimer();m&&a.fixedToolbars.hide(true)}}}).bind("scrollstop",function(m){if(!a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length){s=false;if(e){e=false;a.fixedToolbars.startShowTimer()}q=null}}).bind("silentscroll",c);a(window).bind("resize",c)});a(".ui-page").live("pagebeforeshow",function(m){m=a(m.target).find('[data-role="footer"]:not(.ui-sticky-footer)');var r=m.data("id");
...
a(this),t=a(window).scrollTop(),v=f(n[0]),l=window.innerHeight,p=n.outerHeight();t=n.is(".ui-header-fixed")&&t<=v+p||n.is(".ui-footer-fixed")&&v<=t+l;n.addClass("ui-fixed-overlay").removeClass("ui-fixed-inline");!t&&!m&&n.animationComplete(function(){n.removeClass("in")}).addClass("in");b(n)})},hide:function(m){g="inline";return(a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var r=a(this),
...
clearTimeout(i);i=d},toggle:function(m){if(m)g=m;return g=="overlay"?a.fixedToolbars.hide():a.fixedToolbars.show()},setTouchToggleEnabled:function(m){w=m}}}}()})(jQuery);
...
g+"'></div>");b.bind({mouseover:function(){if(a(this).parent().is(".ui-disabled"))return false},touchmove:function(i){i=i.originalEvent.touches[0];if(b.data("movestart")){if(Math.abs(b.data("movestart")[0]-i.pageX)>10||Math.abs(b.data("movestart")[1]-i.pageY)>10)b.data("moved",true)}else b.data("movestart",[parseFloat(i.pageX),parseFloat(i.pageY)])},"touchend mouseup":function(i){if(!f.is(":disabled")){b.removeData("movestart");if(b.data("etype")&&b.data("etype")!==i.type||b.data("moved")){b.removeData("etype").removeData("moved");
...
"aria-haspopup":"true","aria-owns":j})).text(a(f[0].options.item(f[0].selectedIndex)).text()).insertBefore(f).buttonMarkup({theme:c.theme,icon:c.icon,iconpos:c.iconpos,inline:c.inline,corners:c.corners,shadow:c.shadow,iconshadow:c.iconshadow}),i=d.isMultiple=f[0].multiple;c.nativeMenu&&window.opera&&window.opera.version&&f.addClass("ui-select-nativeonly");if(!c.nativeMenu){var h=f.find("option"),k=b+"-button",j=b+"-menu",o=f.closest(".ui-page"),q=/ui-btn-up-([a-z])/.exec(e.attr("class"))[1],s=a("<div data-role='dialog' data-theme='"+
c.menuPageTheme+"'><div data-role='header'><div class='ui-title'>"+g.text()+"</div></div><div data-role='content'></div></div>").appendTo(a.mobile.pageContainer).page(),w=s.find(".ui-content"),m=s.find(".ui-header a"),r=a("<div>",{"class":"ui-selectmenu-screen ui-screen-hidden"}).appendTo(o),n=a("<div>",{"class":"ui-selectmenu ui-selectmenu-hidden ui-overlay-shadow ui-corner-all pop ui-body-"+c.overlayTheme}).insertAfter(r),t=a("<ul>",{"class":"ui-selectmenu-list",id:j,role:"listbox","aria-labelledby":k,
"data-theme":q}).appendTo(n);q=a("<div>",{"class":"ui-header ui-bar-"+q}).prependTo(n);var v=a("<h1>",{"class":"ui-title"}).appendTo(q),l=a("<a>",{"data-iconpos":"notext","data-icon":"delete",text:c.closeText,href:"#","class":"ui-btn-left"}).appendTo(q).buttonMarkup()}if(i)d.buttonCount=a("<span>").addClass("ui-li-count ui-btn-up-c ui-btn-corner-all").hide().appendTo(e);c.disabled&&this.disable();f.change(function(){d.refresh()});a.extend(d,{select:f,optionElems:h,selectID:b,label:g,buttonId:k,menuId:j,
thisPage:o,button:e,menuPage:s,menuPageContent:w,screen:r,listbox:n,list:t,menuType:void 0,header:q,headerClose:l,headerTitle:v,placeholder:""});if(c.nativeMenu){f.appendTo(e).bind("touchstart mousedown",function(){e.addClass(a.mobile.activeBtnClass)}).bind("focus mouseover",function(){e.trigger("mouseover")}).bind("touchmove",function(){e.removeClass(a.mobile.activeBtnClass)}).bind("change blur mouseout",function(){e.trigger("mouseout").removeClass(a.mobile.activeBtnClass)});e.attr("tabindex","-1")}else{d.refresh();
f.attr("tabindex","-1").focus(function(){a(this).blur();e.focus()});e.bind("touchstart",function(p){a(this).data("startTouches",a.extend({},p.originalEvent.touches[0]))}).bind(a.support.touch?"touchend":"mouseup",function(p){a(this).data("moved")?a(this).removeData("moved"):d.open();p.preventDefault()}).bind("touchmove",function(p){p=p.originalEvent.touches[0];var u=a(this).data("startTouches"),A=Math.abs(p.pageY-u.pageY);if(Math.abs(p.pageX-u.pageX)>10||A>10)a(this).data("moved",true)});t.delegate("li:not(.ui-disabled, .ui-li-divider)",
"click",function(p){if(a(p.target).is("a")){var u=t.find("li:not(.ui-li-divider)").index(this);u=d.optionElems.eq(u)[0];u.selected=i?!u.selected:true;i&&a(this).find(".ui-icon").toggleClass("ui-icon-checkbox-on",u.selected).toggleClass("ui-icon-checkbox-off",!u.selected);f.trigger("change");i||d.close();p.preventDefault()}});r.add(l).add(m).bind("click",function(p){d.close();p.preventDefault();a.contains(m[0],p.target)&&p.stopImmediatePropagation()})}},_buildList:function(){var d=this,c=this.options,
f=this.placeholder,b=[],g=[],e=d.isMultiple?"checkbox-off":"false";d.list.empty().filter(".ui-listview").listview("destroy");d.select.find("option").each(function(){var i=a(this),h=i.parent(),k=i.text(),j="<a href='#'>"+k+"</a>",o=[],q=[];if(h.is("optgroup")){h=h.attr("label");if(a.inArray(h,b)===-1){g.push("<li data-role='list-divider'>"+h+"</li>");b.push(h)}}if(!this.getAttribute("value")||k.length==0||i.data("placeholder")){c.hidePlaceholderMenuItems&&o.push("ui-selectmenu-placeholder");f=d.placeholder=
k}if(this.disabled){o.push("ui-disabled");q.push("aria-disabled='true'")}g.push("<li data-icon='"+e+"' class='"+o.join(" ")+"' "+q.join(" ")+">"+j+"</li>")});d.list.html(g.join(" "));this.isMultiple||this.headerClose.hide();!this.isMultiple&&!f.length?this.header.hide():this.headerTitle.text(this.placeholder);d.list.listview()},refresh:function(d){var c=this,f=this.element,b=this.isMultiple,g=this.optionElems=f.find("option"),e=g.filter(":selected"),i=e.map(function(){return g.index(this)}).get();
...
var k=function(){return e=="input"?parseFloat(c.val()):c[0].selectedIndex},j=e=="input"?parseFloat(c.attr("min")):0,o=e=="input"?parseFloat(c.attr("max")):c.find("option").length-1,q=window.parseFloat(c.attr("step")||1),s=a('<div class="ui-slider '+f+" ui-btn-down-"+g+' ui-btn-corner-all" role="application"></div>'),w=a('<a href="#" class="ui-slider-handle"></a>').appendTo(s).buttonMarkup({corners:true,theme:b,shadow:true}).attr({role:"slider","aria-valuemin":j,"aria-valuemax":o,"aria-valuenow":k(),
"aria-valuetext":k(),title:k(),"aria-labelledby":h});a.extend(this,{slider:s,handle:w,dragging:false,beforeStart:null});if(e=="select"){s.wrapInner('<div class="ui-slider-inneroffset"></div>');c.find("option");c.find("option").each(function(m){var r=m==0?"b":"a",n=m==0?"right":"left";m=m==0?" ui-btn-down-"+g:" ui-btn-active";a('<div class="ui-slider-labelbg ui-slider-labelbg-'+r+m+" ui-btn-corner-"+n+'"></div>').prependTo(s);a('<span class="ui-slider-label ui-slider-label-'+r+m+" ui-btn-corner-"+
n+'" role="img">'+a(this).text()+"</span>").prependTo(w)})}i.addClass("ui-slider");c.addClass(e=="input"?"ui-slider-input":"ui-slider-switch").change(function(){d.refresh(k(),true)}).keyup(function(){d.refresh(k(),true,true)}).blur(function(){d.refresh(k(),true)});a(document).bind("touchmove mousemove",function(m){if(d.dragging){d.refresh(m);return false}});s.bind("touchstart mousedown",function(m){d.dragging=true;if(e==="select")d.beforeStart=c[0].selectedIndex;d.refresh(m);return false});s.add(document).bind("touchend mouseup",
function(){if(d.dragging){d.dragging=false;if(e==="select"){if(d.beforeStart===c[0].selectedIndex)d.refresh(d.beforeStart===0?1:0);var m=k();m=Math.round(m/(o-j)*100);w.addClass("ui-slider-handle-snapping").css("left",m+"%").animationComplete(function(){w.removeClass("ui-slider-handle-snapping")})}return false}});s.insertAfter(c);this.handle.bind("touchstart mousedown",function(){a(this).focus()});this.handle.bind("keydown",function(m){var r=k();if(!d.options.disabled){switch(m.keyCode){case a.mobile.keyCode.HOME:case a.mobile.keyCode.END:case a.mobile.keyCode.PAGE_UP:case a.mobile.keyCode.PAGE_DOWN:case a.mobile.keyCode.UP:case a.mobile.keyCode.RIGHT:case a.mobile.keyCode.DOWN:case a.mobile.keyCode.LEFT:m.preventDefault();
if(!d._keySliding){d._keySliding=true;a(this).addClass("ui-state-active")}}switch(m.keyCode){case a.mobile.keyCode.HOME:d.refresh(j);break;case a.mobile.keyCode.END:d.refresh(o);break;case a.mobile.keyCode.PAGE_UP:case a.mobile.keyCode.UP:case a.mobile.keyCode.RIGHT:d.refresh(r+q);break;case a.mobile.keyCode.PAGE_DOWN:case a.mobile.keyCode.DOWN:case a.mobile.keyCode.LEFT:d.refresh(r-q)}}}).keyup(function(){if(d._keySliding){d._keySliding=false;a(this).removeClass("ui-state-active")}});this.refresh()},
...
return false}})},_itemApply:function(d,c){c.find(".ui-li-count").addClass("ui-btn-up-"+(d.data("counttheme")||this.options.countTheme)+" ui-btn-corner-all");c.find("h1, h2, h3, h4, h5, h6").addClass("ui-li-heading");c.find("p, dl").addClass("ui-li-desc");d.find("li").find("img:eq(0)").addClass("ui-li-thumb").each(function(){a(this).closest("li").addClass(a(this).is(".ui-li-icon")?"ui-li-has-icon":"ui-li-has-thumb")});var f=c.find(".ui-li-aside");f.length&&f.each(function(b,g){a(g).prependTo(a(g).parent())});
...
e.first().attr("tabindex","0");e.each(function(h){var k=a(this),j="ui-li";if(!(!d&&k.hasClass("ui-li"))){var o=k.data("theme")||c.theme,q=k.find("a");if(q.length){var s=k.data("icon");k.buttonMarkup({wrapperEls:"div",shadow:false,corners:false,iconpos:"right",icon:q.length>1||s===false?false:s||"arrow-r",theme:o});q.first().addClass("ui-link-inherit");if(q.length>1){j+=" ui-li-has-alt";q=q.last();s=f.data("splittheme")||q.data("theme")||c.splitTheme;q.appendTo(k).attr("title",q.text()).addClass("ui-li-link-alt").empty().buttonMarkup({shadow:false,
corners:false,theme:o,icon:false,iconpos:false}).find(".ui-btn-inner").append(a("<span>").buttonMarkup({shadow:true,corners:true,theme:s,iconpos:"notext",icon:f.data("spliticon")||q.data("icon")||c.splitIcon}))}}else if(k.data("role")==="list-divider"){j+=" ui-li-divider ui-btn ui-bar-"+g;k.attr("role","heading");if(i)i=1}else j+=" ui-li-static ui-btn-up-"+o;if(c.inset){if(h===0){j+=" ui-corner-top";k.add(k.find(".ui-btn-inner")).find(".ui-li-link-alt").addClass("ui-corner-tr").end().find(".ui-li-thumb").addClass("ui-corner-tl");
...
"-")},_createSubPages:function(){var d=this.element,c=d.closest(".ui-page"),f=c.data("url"),b=this.options,g=this,e=c.find("[data-role='footer']").data("id");a(d.find("ul, ol").toArray().reverse()).each(function(i){var h=a(this),k=h.parent(),j=a.trim(k.contents()[0].nodeValue)||k.find("a:first").text();i=f+"&"+a.mobile.subPageUrlKey+"="+g._idStringEscape(j+" "+i);var o=h.data("theme")||b.theme,q=h.data("counttheme")||d.data("counttheme")||b.countTheme;h.wrap("<div data-role='page'><div data-role='content'></div></div>").parent().before("<div data-role='header' data-theme='"+
b.headerTheme+"'><div class='ui-title'>"+j+"</div></div>").after(e?a("<div>",{"data-role":"footer","data-id":e,"class":"ui-footer-duplicate"}):"").parent().attr({"data-url":i,"data-theme":o,"data-count-theme":q}).appendTo(a.mobile.pageContainer).page();h=k.find("a:first");h.length||(h=a("<a></a>").html(j).prependTo(k.empty()));h.attr("href","#"+i)}).listview()}})})(jQuery);
(function(a){a.mobile.listview.prototype.options.filter=false;a("[data-role='listview']").live("listviewcreate",function(){var d=a(this);if(d.data("listview").options.filter){var c=a("<form>",{"class":"ui-listview-filter ui-bar-c",role:"search"});a("<input>",{placeholder:"Filter results...","data-type":"search"}).bind("keyup change",function(){var f=this.value.toLowerCase(),b=d.children();b.show();if(f)for(var g=false,e,i=b.length;i>=0;i--){e=a(b[i]);if(e.is("li[data-role=list-divider]")){g||e.hide();
g=false}else if(e.text().toLowerCase().indexOf(f)===-1)e.hide();else g=true}}).appendTo(c).textinput();c.insertBefore(d)}})})(jQuery);
...
(function(a,d){var c=a("html"),f=a("head"),b=a(d);a.extend(a.mobile,{initializePage:function(){var g=a("[data-role='page']");g.add("[data-role='dialog']").each(function(){var e=a(this);e.data("url")||e.attr("data-url",e.attr("id"))});a.mobile.firstPage=g.first();a.mobile.pageContainer=g.first().parent().addClass("ui-mobile-viewport");a.mobile.pageLoading();!a.mobile.hashListeningEnabled||!a.mobile.path.stripHash(location.hash)?a.mobile.changePage(a.mobile.firstPage,false,true,false,true):b.trigger("hashchange",
[true])}});a(d.document).trigger("mobileinit");if(a.mobile.gradeA()){c.addClass("ui-mobile ui-mobile-rendering");a.mobile.metaViewportContent&&a("<meta>",{name:"viewport",content:a.mobile.metaViewportContent}).prependTo(f);a(a.mobile.initializePage);b.load(a.mobile.silentScroll)}})(jQuery,this);

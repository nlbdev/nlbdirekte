...
def setAttribute(elem, attr, value):
	if (isinstance(attr, str) and isinstance(elem, list) and len(elem) >= 1):
		if (len(elem) == 1):
...
for child in body:
	...
	id = ""
	if (alreadyThere >= 0):
		id = 'smil'+str(alreadyThere)+'_'+smil[2]
	else:
		id = 'smil'+str(len(theFlow))+'_'+smil[2]
...
for currentFlow in range(len(theFlow)):
	smilFile = open(theFlow[currentFlow]['smil'].rpartition('/')[2]+".json")
...
def fixTimes(parent, current, siblingNr):
	...
	type = str(getAttribute(current, 't', ''))
		...
		if (not determinedSomething):
			...
			if (begin < 0 and parent != None and (siblingNr == 0 or parent[0] == 'p')):
				if (getAttribute(parent,'b',-1) >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('begin '+str(begin)+' #8')
			...
			if (begin < 0 and siblingNr > 0 and parent != None and parent[0] == 's'):
				if (getAttribute(getChild(parent,siblingNr-1),'e',-1) >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('begin '+str(begin)+' #9')
			...
			if (begin < 0 and parent != None and parent[0] == 'p'):
				for s in range(0,numberOfChildren(parent)):
					...
					if (getAttribute(getChild(parent,s),'b',-1) >= 0):
						...
						determinedSomething = True
						if (ftDebug): print('begin '+str(begin)+' #10')
						break
			...
			if (begin < 0 and current[0] == 'p'):
				for c in range(0,numberOfChildren(current)):
					if (getAttribute(getChild(current,c),'b',-1) >= 0):
						...
						determinedSomething = True
						if (ftDebug): print('begin '+str(begin)+' #11')
						break
			...
			if (begin < 0 and current[0] == 's'):
				if (numberOfChildren(current) > 0 and getAttribute(getChild(current,0),'b',-1) >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('begin '+str(begin)+' #12')
			...
			if (end < 0 and current[0] == 'p'):
				...
				if (newEnd >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('end '+str(end)+' #13')
			...
			if (end < 0 and current[0] == 's'):
				if (numberOfChildren(current) > 0 and getAttribute(getChild(current,numberOfChildren(current)-1),'e',-1) >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('end '+str(end)+' #14')
			...
			if (end < 0 and parent != None and parent[0] == 's' and siblingNr == numberOfChildren(parent)-1):
				if (getAttribute(parent,'e',-1) >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('end '+str(end)+' #15')
			...
			if (end < 0 and parent != None and parent[0] == 's' and siblingNr < numberOfChildren(parent)-1):
				if (getAttribute(getChild(parent,siblingNr+1),'b',-1) >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('end '+str(end)+' #16')
			...
			if (end < 0 and parent != None and parent[0] == 'p'):
				...
				if (newEnd >= 0):
					...
					determinedSomething = True
					if (ftDebug): print('end '+str(end)+' #17')
				...
				for s in range(siblingNr-1,-1,-1):
					if (getAttribute(current,'s','').partition('#')[0] == getAttribute(getChild(parent,s),'s','').partition('#')[0]):
						if (getAttribute(getChild(parent,s),'E',-1) >= 0):
							...
							determinedSomething = True
							if (ftDebug): print('clipBegin '+str(clipBegin)+' #18')
						break
				...
				for s in range(siblingNr+1,numberOfChildren(parent)):
					if (getAttribute(current,'s','').partition('#')[0] == getAttribute(getChild(parent,s),'s','').partition('#')[0]):
						if (getAttribute(getChild(parent,s),'B',-1) >= 0):
							...
							determinedSomething = True
							if (ftDebug): print('clipEnd '+str(clipEnd)+' #19')
						break
		...
		if (not determinedSomething):
			for child in range(0,numberOfChildren(current)):
				if (fixTimes(current, getChild(current,child), child)):
					determinedSomething = True
					if (ftDebug): print('recursive '+str(getChild(current,child)[0])+' #20')
...
for i in range(0,numberOfChildren(smil)):
	prependId(getChild(smil,i),'smil'+str(i)+'_')

def postProcess(current):
	# check pagelist
	for i in range(1,len(pagelist)):
		if (getAttribute(pagelist[i],'i','0') == getAttribute(current,'i','1')):
			setAttribute(pagelist[i],'b',str(round(float(getAttribute(current,'b',"-1")),4)))
			setAttribute(pagelist[i],'e',str(round(float(getAttribute(current,'e',"-1")),4)))
			break
	
	# check toc
	for i in range(1,len(toc)):
		if (getAttribute(toc[i],'i','0') == getAttribute(current,'i','1')):
			setAttribute(toc[i],'b',str(round(float(getAttribute(current,'b',"-1")),4)))
			setAttribute(toc[i],'e',str(round(float(getAttribute(current,'e',"-1")),4)))
			break
	...
	if (hasAttribute(current,'b') and getAttribute(current,'b',-1) != -1):
		setAttribute(current,'b',str(round(float(getAttribute(current,'b',-1)),4)))
	#else: removeAttribute(current,'b')
	
	if (hasAttribute(current,'e') and getAttribute(current,'e',-1) != -1):
		setAttribute(current,'e',str(round(float(getAttribute(current,'e',-1)),4)))
	...
	if (hasAttribute(current,'d') and getAttribute(current,'d',-1) != -1):
		setAttribute(current,'d',str(round(float(getAttribute(current,'d',-1)),4)))
...
smilFile = open("smil.json", 'w')
json.dump(smil, smilFile, indent=4)
smilFile.close()

tocFile = open("toc.json", 'w')
json.dump(toc, tocFile)
tocFile.close()

pagelistFile = open("pagelist.json", 'w')
json.dump(pagelist, pagelistFile)
pagelistFile.close()
